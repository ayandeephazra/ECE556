                  ...
                  Node *current, Node T)
{
  ...
  if (leftX >= 0)
  {
    ...
    calcG(tmp);
    tmp.F = tmp.G + tmp.distToT * (1.0 + prevAvgCost);
...
int maze(routingInst *rst, unordered_map<point, Node *, PointHash> openSet, priority_queue<Node *, vector<Node *>, CompareNodeCost> openSet_pq,
         unordered_map<point, Node *, PointHash> closedSet, Node nT, Node *nS, int netInd, int SpinInd)
{

  // we start maze routing while we still have open nodes in the pq
  while (!openSet_pq.empty())
  {
    // we pull the lowest cost node from the pq
         ...
         neighbor++)
    {
      ...
      if (elem != openSet.end())
      {
        ...
        calcG(*elem->second);
        (*elem->second).F = (*elem->second).G + (*elem->second).distToT * (1.0 + prevAvgCost);
...
int routeNetAstar(routingInst *rst, int netInd, int SpinInd, int TpinInd)
{
  ...
  //
  int r = maze(rst, openSet, openSet_pq, closedSet, nT, nS, netInd, SpinInd);
  if (r == EXIT_SUCCESS)
  {
    return EXIT_SUCCESS;
  }
